...
tf.app.flags.DEFINE_string('w2v_file', 'data_en/w2v_200.txt', 'embedding file')
...
def run():
    ...
    # Model Code Block
    word_idx_rev, word_id_mapping, word_embedding, pos_embedding = load_w2v(FLAGS.embedding_dim, FLAGS.embedding_dim_pos, 'data_combine_en/clause_keywords.csv', FLAGS.w2v_file)
    ...
    with tf.Session(config=tf_config) as sess:
        ...
        for fold in range(1,11):
            ...
            # Data Code Block
            train_file_name = 'fold_{}_train.txt'.format(fold)
            test_file_name = 'fold_{}_test.txt'.format(fold)
            tr_doc_id, tr_y_position, tr_y_cause, tr_y_pairs, tr_x, tr_sen_len, tr_doc_len = load_data('data_combine_en/'+train_file_name, word_id_mapping, FLAGS.max_doc_len, FLAGS.max_sen_len)
            te_doc_id, te_y_position, te_y_cause, te_y_pairs, te_x, te_sen_len, te_doc_len = load_data('data_combine_en/'+test_file_name, word_id_mapping, FLAGS.max_doc_len, FLAGS.max_sen_len)
